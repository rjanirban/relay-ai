...
// Relay AI presentation:
import { RelayConversationLayout } from './relay/RelayConversationLayout';
import { RelayTranscriptPanel } from './relay/RelayTranscriptPanel';
import { RelayIntelligencePanel } from './relay/RelayIntelligencePanel';
import { RelayEscalationPanel } from './relay/RelayEscalationPanel';
import { rollupRelayTranscript, shouldEscalate } from '@/lib/relayIntelligence';
...
export default function ConversationComponent({
  ...
  // Idempotent shutdown: latch true on the first End click so the button disables
  // immediately and no duplicate stop request (or 409 conflict) can surface.
  ...
  // Relay AI: derive an HONEST intelligence rollup from the real Agora transcript.
  // Nothing here is fabricated — absent data yields explicit "pending/not-detected".
  const relayRollup = useMemo(
    () =>
      rollupRelayTranscript(
        transcript,
        String(client.uid),
        String(agentUID),
      ),
    [transcript, client.uid, agentUID],
  );
  const relayEscalationReason = useMemo(
    () => shouldEscalate(relayRollup),
    [relayRollup],
  );

  // Relay AI scalar/vector labels shown in the escalation flow. Computed only when a
  // handoff is confirmed so the brief reflects the exact moment of transfer.
  const [relayCaseId, setRelayCaseId] = useState<string>('RLY----');
  const [transferring, setTransferring] = useState(false);

  const relayCurrentUpdate = useMemo<RelaySessionUpdate>(
    () => ({
      rollup: relayRollup,
      escalationReason: relayEscalationReason,
  ...
  // Lifted intelligence used by the host. Prism keeps the rollup recomputing from
  // real events while the snapshot below (escalation) is frozen at transfer time.
  useEffect(() => {
    if (!onIntelligenceUpdate) return;
    onIntelligenceUpdate(relayCurrentUpdate);
  }, [onIntelligenceUpdate, relayCurrentUpdate]);
  ...
  const handleEscalate = useCallback(async () => {
    ...
    try {
      // Host records the case using its own snapshot + returns a case id, or we
      // derive a local one. Context (transcript/intent/confidence) is preserved via
      // the frozen brief rendered by RelayEscalationPanel.
      const caseId = onEscalate?.(relayCurrentUpdate) ?? null;
      setRelayCaseId(caseId ?? `RLY-${Date.now().toString().slice(-6)}`);
    } finally {
      setTransferring(false);
    }
  }, [transferring, onEscalate, relayCurrentUpdate]);
  ...
  const handleEndConversation = useCallback(async () => {
    // Idempotent shutdown: guard against double-invocation (e.g. double-click or a
    // second click while the stop request is in flight). The backend already treats
    // a 409 "already in the process of shutting down" as success; latching here
    // prevents us from ever *sending* that duplicate request in the first place.
    if (isEnding) return;
    setIsEnding(true);
    try {
      await onEndConversation();
    } finally {
      // Keep the button disabled in the "ending" state; the host resets it when a
      // new conversation is started.
      setIsEnding(false);
    }
  }, [isEnding, onEndConversation]);

  return (
    <RelayConversationLayout
      statusPanel={
      ...
      visualizer={
        <div
          ...
          role="region"
          aria-label="Relay AI agent status visualization"
        >
